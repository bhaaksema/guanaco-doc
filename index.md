...
All axioms and rules are from [Meyer & Hoek (1995)](#references) and [Van Ditmarsch et al. (2007)](#references). Note that $\mathbf{A2}$ and $\mathbf{A2}'$ are propositionally equivalent, and so are $\mathbf{A9}$ and $\mathbf{A9}'$. Also note that Guanaco cannot check whether a formula is a propositional tautology. Users can select $\mathbf{A1}$, but if they do so, Guanaco will not validate this justification. This limitation is due to time constraints, and to keep our focus on the Epistemic challenges. For now, the responsibility to check whether a formula is a propositional tautology lies with the user. There are several tautology checkers available online, though the user may have to translate their formula to a scheme that the tautology checker understands. Finally, $A6$ is available only if the amount of $m$ is specified by the user.
...
- **Guanaco cannot check whether a formula is a propositional tautology.** So Guanaco will not validate any application of $\mathbf{A1}$. The user must use an external service to check whether a formula is indeed a propositional tautology.
